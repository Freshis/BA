...
\empty
    ...
    {\huge \textcolor{red}{Simulation der Radialen-Basisfunktion-Finite-Differenzen-Methode angewendet auf die Transportgleichung und Stabilisierung mittels Flux Correction Tools}}
    \vspace*{2.5cm}
...
\begin{align}
 (K^Hu)_i=\sum_{j}k_{ij}^Hu_j&=0\nonumber\\
 \Rightarrow\sum_{j\neq i}k_{ij}^Hu_j&=-k_{ii}^Hu_i\nonumber\\
 \Rightarrow(K^Hu)_i&=\sum_{j\neq i}k_{ij}^H(u_j-u_i),~~\forall i\nonumber
\end{align}
